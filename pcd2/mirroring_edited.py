...
def rotate_and_mirror_image(image, degree, mirror=True):
    ...
    if mirror:
        output_image = np.flipud(np.fliplr(output_image))  # Vertical and horizontal flip

    return output_image

# Load the image
image_path = "C:\\Users\\muham\\Downloads\\LOGO SMK 4 KOTA SUKABUMI.jpg"
image = img.imread(image_path)

# Rotate the image by 45 degrees and apply mirroring
rotated_mirrored_image = rotate_and_mirror_image(image, 45, mirror=True)

# Display the original and mirrored-rotated images
plt.subplot(1, 2, 1)
plt.imshow(image)
plt.title("Original Image")

plt.subplot(1, 2, 2)
plt.imshow(rotated_mirrored_image)
plt.title("Rotated and Mirrored Image")
